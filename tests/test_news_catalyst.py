...
class TestNewsCatalyst:
    @patch("src.agents.news_catalyst._finnhub_client")
    @patch("src.agents.news_catalyst.ChatGoogleGenerativeAI")
    def test_detects_catalyst(self, mock_llm_class, mock_finnhub):
        # Mock Finnhub returns news
        mock_finnhub.company_news.return_value = [
            {"headline": "CHIPS Act funding approved", "summary": "Major boost for semiconductor industry"}
        ]
    ...
    @patch("src.agents.news_catalyst._finnhub_client")
    def test_no_news_no_signal(self, mock_finnhub):
        mock_finnhub.company_news.return_value = []
